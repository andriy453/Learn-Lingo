...
const BookForm = ({ setIsBookModalOpened, teacherDataForBook, color }) => {
  ...
  const schema = Yup.object({
    ...
    email: Yup.string()
      ...
      phone: Yup.string().matches(/^\+\d{12}$/, 'Phone must have + and length 12').min(13, 'Phone Too Short!').max(13, "Phone Too long").required('Required'),
  });
  ...
  return (
    <RegisterFormStyled onSubmit={formik.handleSubmit}>
          ...
          <ButonClosedModal onClick={hendelClikClosed}>
              <Svgx><use href={sprite + "#icon-x"}></use></Svgx>
          </ButonClosedModal>
    </RegisterFormStyled>
  );
};
